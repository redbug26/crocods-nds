...
int AMSDOS_GetSectorIndex(int nDrive, int nTrack, int nID)
{
	...
	do {
//		myprintf("CHRN %02X%02X%02X%02X", C,H,R,N);
		if ((N == 0x02) && (R == nID) && (H==0)) {
//			myprintf("I: %d", IndexSecteur);
			return IndexSecteur;
		}
		ReadCHRN();
...
BOOL AMSDOS_GetSector(int nDrive, int nTrack, int nSector, unsigned char *pBuffer)
{
	...
	nSectorIndex = RechercheSecteur( nSector, &newPos );
	...
	ChangeCurrTrack(nTrack);
//	myprintf("PosData: %d", PosData);
	
//	nSectorIndex=nSector;
	
//	myprintf("Found: %d,%d,%d", cntdata, PosData, TailleSect);
//	myprintf("Found: %X,%d", sizeof(CPCEMUEnt) + cntdata + PosData, TailleSect);
	
	memcpy(pBuffer, ImgDsk + cntdata + PosData, TailleSect);
	
//	myprintf("pB: %02X%02X%02X%02X", pBuffer[0], pBuffer[1], pBuffer[2], pBuffer[3]);
//	myprintf("pB: %02X%02X%02X%02X", ImgDsk[0], ImgDsk[1], ImgDsk[2], ImgDsk[3]);
...
BOOL AMSDOS_GenerateAutorunCommand(char *AutorunCommand)
{
